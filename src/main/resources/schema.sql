-- KPL 电竞数据分析 Agent 数据库建表脚本
CREATE DATABASE IF NOT EXISTS kpl_agent DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE kpl_agent;

-- 赛事表
CREATE TABLE IF NOT EXISTS league (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    league_id VARCHAR(32) NOT NULL COMMENT '赛事ID(如20260001)',
    league_name VARCHAR(128) NOT NULL COMMENT '赛事名称',
    league_type VARCHAR(32) COMMENT '赛事类型(kpl/world_champion_cup等)',
    year INT COMMENT '年份',
    season INT COMMENT '赛季(1春/2夏/3年度/4挑战者杯)',
    cc_league_id VARCHAR(32) COMMENT '内部赛事ID(如KPL2026S1)',
    start_time DATETIME COMMENT '开始时间',
    end_time DATETIME COMMENT '结束时间',
    status INT DEFAULT 0 COMMENT '状态(0未知/1进行中/2已结束)',
...
CREATE TABLE IF NOT EXISTS `match` (
    ...
    bo INT COMMENT 'BO数(3/5/7)',
    win_camp INT COMMENT '获胜方(1/2)',
    status INT DEFAULT 0 COMMENT '状态(0未开始/1进行中/2已结束)',
    match_stage VARCHAR(32) COMMENT '赛段(cgs1/cgs2/jhs等)',
...
CREATE TABLE IF NOT EXISTS battle (
    ...
    win_camp INT COMMENT '获胜方(1/2)',
    game_duration BIGINT COMMENT '对局时长(毫秒)',
